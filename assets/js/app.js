...
(function () {
  ...
  function viewCategory(tabId, query, selectedTag) {
    ...
    return `
      ...
      ${canonicalTabId(tabId) === 'Magias' ? spellRegistrySection() : ''}
    `;
  }
  ...
  /* ── MAGIAS CRIADAS (localStorage) ────────────── */
  const SPELLS_LS_KEY    = 'arcano_user_spells';
  ...
  const AFFINITY_OPTIONS = ['Elemental', 'Arcanista', 'Druídico', 'Celestial'];
  const SCOPE_OPTIONS    = ['Dano', 'Cura', 'Buff', 'Debuff', 'Controle', 'Utilidade'];
  const SCOPE_COLORS = {
    Dano: '#f43f5e', Cura: '#10b981', Buff: '#3b82f6',
    Debuff: '#f59e0b', Controle: '#a855f7', Utilidade: '#9ca3af'
  };
  const AFFINITY_META = {
    Elemental: { color: '#fb923c', icon: '🔥' },
    Arcanista: { color: '#22d3ee', icon: '✦' },
    'Druídico': { color: '#84cc16', icon: '🌿' },
    Celestial: { color: '#fbbf24', icon: '✨' }
  };

  function loadUserSpells() {
    try {
      const arr = JSON.parse(localStorage.getItem(SPELLS_LS_KEY));
      return Array.isArray(arr) ? arr : [];
    } catch { return []; }
  }
  function saveUserSpells(list) {
    try { localStorage.setItem(SPELLS_LS_KEY, JSON.stringify(list)); } catch (err) {
      console.error('Não foi possível salvar magias:', err);
    }
  }
  function userSpellById(id) {
    return loadUserSpells().find((s) => s.id === id) || null;
  }

  function spellScopePills(scopes) {
    return (scopes || []).map((s) =>
      `<span class="spell-pill" style="--pill:${SCOPE_COLORS[s] || '#9ca3af'}">${escapeHtml(s)}</span>`
    ).join('');
  }

  function spellFichaHTML(s, i) {
    const aff = AFFINITY_META[s.affinity] || { color: 'var(--primary-2)', icon: '✦' };
    const area = (s.areaW && s.areaH) ? `${escapeHtml(s.areaW)} × ${escapeHtml(s.areaH)} q` : '—';
    const row = (icon, label, value, cls) => `
      <div class="spell-row">
        <span class="spell-row__label">${icon} ${escapeHtml(label)}</span>
        <span class="spell-row__value ${cls || ''}">${value}</span>
      </div>`;
    return `
      <article class="spell-ficha" style="--accent:${aff.color};--delay:${i * 50}ms">
        <header class="spell-ficha__head">
          <span class="spell-ficha__icon">${aff.icon}</span>
          <h4 class="spell-ficha__name">${escapeHtml(s.name)}</h4>
          <div class="spell-ficha__actions">
            <button type="button" class="spell-act" data-spell-edit="${escapeHtml(s.id)}" title="Editar" aria-label="Editar magia">
            ...
            <button type="button" class="spell-act spell-act--danger" data-spell-del="${escapeHtml(s.id)}" title="Deletar" aria-label="Deletar magia">
        ...
        ${(s.scopes && s.scopes.length) ? `<div class="spell-ficha__scopes">${spellScopePills(s.scopes)}</div>` : ''}
        <div class="spell-ficha__rows">
          ${row('💧', 'Custo', `<b>${escapeHtml(String(s.cost))} MP</b>`, 'is-cost')}
          ${row('✦', 'Tier', escapeHtml(s.tier), 'is-tier')}
          ${row('🎯', 'Alcance', `${escapeHtml(String(s.range))} q`, 'is-neutral')}
          ${row('▦', 'Área', area, 'is-neutral')}
          ${row('⏳', 'Duração', escapeHtml(s.duration), 'is-duration')}
          ${row(aff.icon, 'Afinidade', `<b>${escapeHtml(s.affinity)}</b>`, 'is-affinity')}
        </div>
        ${s.description ? `<p class="spell-ficha__desc">${escapeHtml(s.description)}</p>` : ''}
        ${s.extra ? `<div class="spell-ficha__extra"><span class="spell-ficha__extra-label">Efeito adicional</span>${escapeHtml(s.extra)}</div>` : ''}
      </article>`;
  }

  function spellRegistrySection() {
    const spells = loadUserSpells();
    return `
      <section class="spell-registry" id="spellRegistry" style="--hue:285">
        <div class="spell-registry__head">
          <div class="spell-registry__title">
            <h2>REGISTRO DE MAGIAS</h2>
            <span class="spell-registry__count">${spells.length} ${spells.length === 1 ? 'magia criada' : 'magias criadas'}</span>
          </div>
        </div>

        <div class="spell-registry__sub">
          <span class="spell-registry__sub-label">Magias Criadas</span>
          <span class="spell-registry__line"></span>
        </div>

        ${spells.length
          ? `<div class="spell-grid">${spells.map((s, i) => spellFichaHTML(s, i)).join('')}</div>`
          : `<div class="spell-empty">
               <span class="spell-empty__icon">✦</span>
               <p>Nenhuma magia criada ainda. Forje a primeira abaixo.</p>
             </div>`}

        <div class="spell-registry__foot">
          <button type="button" class="spell-add-btn" data-spell-add>
  ...
  function refreshSpellRegistry() {
    const el = document.getElementById('spellRegistry');
    if (el) el.outerHTML = spellRegistrySection();
  }
  ...
  function spellModalHTML(s) {
    ...
    const scopes = (s && s.scopes) || [];
    return `
      <div class="spell-modal__backdrop" data-spell-close></div>
      <div class="spell-modal__dialog" role="dialog" aria-modal="true" aria-label="${editing ? 'Editar magia' : 'Nova magia'}">
        <header class="spell-modal__head">
          <h3>${editing ? 'Editar Magia' : 'Adicionar Nova Magia'}</h3>
          <button type="button" class="spell-modal__x" data-spell-close aria-label="Fechar">
        ...
        <form class="spell-form" id="spellForm" novalidate>
          <div class="spell-field">
            <label for="spName">Nome</label>
            <input type="text" id="spName" name="name" placeholder="Nome da magia" value="${s ? v(s.name) : ''}" required>
          </div>

          <div class="spell-field-row">
            <div class="spell-field">
              <label for="spCost">Custo</label>
              <input type="number" id="spCost" name="cost" min="0" placeholder="MP" value="${s ? v(s.cost) : ''}" required>
            </div>
            <div class="spell-field">
              <label for="spTier">Tier</label>
              <div class="spell-select">
                <select id="spTier" name="tier" required>
                  <option value="" disabled ${s ? '' : 'selected'}>—</option>
                  ${selectOptionsHTML(TIER_OPTIONS, s && s.tier)}
                </select>
                <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </div>
          </div>
          ...
          <div class="spell-field-row">
            <div class="spell-field">
              <label for="spRange">Alcance</label>
              <input type="number" id="spRange" name="range" min="0" placeholder="Quadrados (1q = 1m)" value="${s ? v(s.range) : ''}" required>
            </div>
            <div class="spell-field">
              <label>Área</label>
              <div class="spell-area">
                <input type="number" id="spAreaW" name="areaW" min="0" placeholder="X" value="${s ? v(s.areaW) : ''}" required>
                <span class="spell-area__x">×</span>
                <input type="number" id="spAreaH" name="areaH" min="0" placeholder="X q" value="${s ? v(s.areaH) : ''}" required>
              </div>
            </div>
          </div>
          ...
          <div class="spell-field-row">
            <div class="spell-field">
              <label for="spDuration">Duração</label>
              <div class="spell-select">
                <select id="spDuration" name="duration" required>
                  <option value="" disabled ${s ? '' : 'selected'}>—</option>
                  ${selectOptionsHTML(DURATION_OPTIONS, s && s.duration)}
                </select>
                <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </div>
            <div class="spell-field">
              <label for="spAffinity">Afinidade</label>
              <div class="spell-select">
                <select id="spAffinity" name="affinity" required>
                  <option value="" disabled ${s ? '' : 'selected'}>—</option>
                  ${selectOptionsHTML(AFFINITY_OPTIONS, s && s.affinity)}
                </select>
                <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </div>
          ...
          <div class="spell-field">
            ...
            <div class="spell-checks">
              ${SCOPE_OPTIONS.map((o) => `
                <label class="spell-check" style="--pill:${SCOPE_COLORS[o]}">
                  <input type="checkbox" name="scope" value="${escapeHtml(o)}" ${scopes.includes(o) ? 'checked' : ''}>
          ...
          <div class="spell-field">
            <label for="spDesc">Descrição</label>
            <textarea id="spDesc" name="description" rows="3" placeholder="Descreva a magia..." required>${s ? v(s.description) : ''}</textarea>
          </div>

          <div class="spell-field">
            <label for="spExtra">Efeito Adicional <span class="spell-field__opt">(opcional)</span></label>
            <textarea id="spExtra" name="extra" rows="2" placeholder="Efeitos especiais, comportamentos condicionais...">${s ? v(s.extra) : ''}</textarea>
          </div>

          <p class="spell-form__error" id="spError" hidden></p>

          <div class="spell-modal__foot">
            <button type="button" class="spell-btn spell-btn--ghost" data-spell-close>Cancelar</button>
            <button type="submit" class="spell-btn spell-btn--save">${editing ? 'Salvar alterações' : 'Salvar'}</button>
          </div>
        </form>
      </div>`;
  }
  ...
  function closeSpellModal() {
    if (!spellModalEl) return;
    spellModalEl.remove();
    spellModalEl = null;
    document.body.style.overflow = '';
    document.removeEventListener('keydown', spellModalEsc);
  }
  function spellModalEsc(e) {
    if (e.key === 'Escape') closeSpellModal();
  }

  function openSpellModal(id) {
    closeSpellModal();
    const spell = id ? userSpellById(id) : null;
    const overlay = document.createElement('div');
    overlay.className = 'spell-modal';
    overlay.innerHTML = spellModalHTML(spell);
    document.body.appendChild(overlay);
    spellModalEl = overlay;
    document.body.style.overflow = 'hidden';
    document.addEventListener('keydown', spellModalEsc);

    overlay.querySelectorAll('[data-spell-close]').forEach((el) =>
      el.addEventListener('click', closeSpellModal));

    const form = overlay.querySelector('#spellForm');
    const errorBox = overlay.querySelector('#spError');
    form.addEventListener('submit', (e) => {
      ...
      const data = {
        name: get('name'), cost: get('cost'), tier: get('tier'),
        range: get('range'), areaW: get('areaW'), areaH: get('areaH'),
        duration: get('duration'), affinity: get('affinity'),
        scopes, description: get('description'), extra: get('extra')
      };

      const missing = [];
      const mark = (sel) => { const el = overlay.querySelector(sel); if (el) el.classList.add('is-invalid'); };
      if (!data.name) { missing.push('Nome'); mark('#spName'); }
      if (!data.cost) { missing.push('Custo'); mark('#spCost'); }
      if (!data.tier) { missing.push('Tier'); mark('#spTier'); }
      if (!data.range) { missing.push('Alcance'); mark('#spRange'); }
      if (!data.areaW) { missing.push('Área'); mark('#spAreaW'); }
      if (!data.areaH) { missing.push('Área'); mark('#spAreaH'); }
      if (!data.duration) { missing.push('Duração'); mark('#spDuration'); }
      if (!data.affinity) { missing.push('Afinidade'); mark('#spAffinity'); }
      if (!scopes.length) { missing.push('Escopo'); mark('.spell-checks'); }
      if (!data.description) { missing.push('Descrição'); mark('#spDesc'); }

      if (missing.length) {
        errorBox.textContent = 'Preencha os campos obrigatórios: ' + [...new Set(missing)].join(', ') + '.';
        errorBox.hidden = false;
        const firstBad = overlay.querySelector('.is-invalid');
        if (firstBad) firstBad.focus();
        return;
      }

      const list = loadUserSpells();
      if (id) {
        const idx = list.findIndex((s) => s.id === id);
        if (idx >= 0) list[idx] = { ...list[idx], ...data };
      } else {
        data.id = 'sp-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 6);
        list.unshift(data);
      }
      saveUserSpells(list);
      closeSpellModal();
      refreshSpellRegistry();
    });

    setTimeout(() => { const f = overlay.querySelector('#spName'); if (f) f.focus(); }, 30);
  }

  function deleteUserSpell(id) {
    const spell = userSpellById(id);
    if (!spell) return;
    if (!confirm(`Apagar a magia "${spell.name}"? Esta ação não pode ser desfeita.`)) return;
    saveUserSpells(loadUserSpells().filter((s) => s.id !== id));
    refreshSpellRegistry();
  }

  let spellRegistryBound = false;
  function attachSpellRegistry() {
    if (spellRegistryBound) return;
    spellRegistryBound = true;
    document.addEventListener('click', (e) => {
      const add = e.target.closest('[data-spell-add]');
      if (add) { openSpellModal(null); return; }
      const edit = e.target.closest('[data-spell-edit]');
      if (edit) { openSpellModal(edit.getAttribute('data-spell-edit')); return; }
      const del = e.target.closest('[data-spell-del]');
      if (del) { deleteUserSpell(del.getAttribute('data-spell-del')); return; }
    });
  }
