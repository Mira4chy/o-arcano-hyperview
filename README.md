...
4. `supabase-master-palette.sql` — paleta de cores personalizada do Mestre (necessaria para salvar cores no editor de texto). Se o site já estiver publicado e mostrar `PGRST205`/`schema cache` ao salvar uma cor, rode este arquivo novamente no SQL Editor: ele cria a tabela se faltar, concede o acesso da Data API e recarrega o esquema.
